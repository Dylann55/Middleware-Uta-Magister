...
const createStudent = new CreateUser();

const createUser = async (req, res) => {
    const dataBase = req.dataBase;
    const { rut, firstName, secondName, surnameM, surnameF, sex, stateCivil, birthday, address, email, phone } = req.body;
    try {
        const status = await createStudent.createUser(dataBase, rut, firstName, secondName, surnameM, surnameF, sex, stateCivil, birthday, address, email, phone)
        if (status != 201) {
